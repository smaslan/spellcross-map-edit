...
{		
	...
	Bind(wxEVT_COMMAND_MENU_SELECTED,&FormANM::OnExportClick,this,wxID_MM_EXPRT);
...
// export
void FormANM::OnExportClick(wxCommandEvent& event)
{
	if(!m_is_pnm || !m_pnm)
		return;
	...
	auto pal = m_spell_data->GetPalette("MAP");
	if(!pal)
		return;
		
	// default name
	auto wname = std::filesystem::path(m_pnm->name).concat(".info").wstring();

	// show save dialog
	wxFileDialog saveFileDialog(this,_("Export animation images"),m_spell_data->export_path,wname,"Animation metadata info file (*.info)|*.info",
		wxFD_SAVE|wxFD_OVERWRITE_PROMPT);
	if(saveFileDialog.ShowModal() == wxID_CANCEL)
		return;
	auto info_path = std::filesystem::path(saveFileDialog.GetPath().ToStdWstring());
	m_spell_data->export_path = saveFileDialog.GetDirectory().ToStdWstring();
	auto dir = info_path.parent_path();

	// save palette
	auto pal_path = (dir / pal->m_name).concat(".palinfo");	
	pal->SaveInfo(pal_path);

	// render surface
	int x_size = m_pnm->x_max;
	int y_size = m_pnm->y_max;
	int y_ref = 0;
	if(m_pnm->y_min < 0)
	{
		y_ref = -m_pnm->y_min;
	...
	for(int frame_id = 0; frame_id < m_pnm->frames.size(); frame_id++)
	{
		auto frame = m_pnm->frames[frame_id];
		...
		// save as png
		auto png_name = string_format("%s_frame_%02d.png",m_pnm->name,frame_id);
		auto png_path = dir / png_name;
	...
	info += string_format("name:: %s.PNM\n",m_pnm->name);
	info += string_format("image:: %s_frame_??.png\n",m_pnm->name);
	info += info_make_text_vector("images", png_names);
	info += string_format("format:: PNM\n");
	info += string_format("pixels:: %d\n",x_size*y_size);
	info += string_format("xsize:: %d\n",x_size);
	info += string_format("ysize:: %d\n",y_size);
	info += string_format("transparent:: %d\n",1);

	auto pal_name = std::filesystem::path(pal->m_name).stem().concat(".palinfo").string();
	info += string_format("palette:: %s\n",pal_name.c_str());

	info += string_format("colors:: ");
	info += pal->GetRangeString();
	info += string_format("\n");

	savestr(info_path, info);
}
...
void FormANM::OnCanvasRepaint(wxPaintEvent& event)
{
	...
	// render flags
	int flags = Terrain::RENDER_CENTER | (cbZoom->IsChecked()?(Terrain::RENDER_ZOOMX2):0);

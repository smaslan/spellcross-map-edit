...
public:
    ...
    SpellMIDI(std::vector<std::filesystem::path>& folders, bool optional=false, std::function<void(std::string)> status_list=NULL, std::function<void(std::string)> status_item=NULL);
    ~SpellMIDI();

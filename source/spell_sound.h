...
public:
    ...
    SpellSounds(FSarchive* common_fs,std::vector<std::filesystem::path>& data_paths, int count=16,std::function<void(std::string)> status_list=NULL,std::function<void(std::string)> status_item=NULL);
    ~SpellSounds();

	...
	if (def)
	{
		...
		for (int k = 0; k < mission_data->Size(); k++)
		{
			...
			else if(cmd->name.compare("AddSpecialEvent") == 0)
			{
				...
				if(!events->GetLastError().empty() && !last_error.empty())
					last_error += "\n";
				last_error += events->GetLastError();
			}
				...
				if(cmd->name.compare(cmd_list[k]) == 0)
				{
					...
					*cmd_dest[k] = cmd->parameters->at(0);
					if(!spelldata->texts->GetText(*cmd_dest[k]))
					{
						...
						spelldata->texts->AddText(*cmd_dest[k],"Text resouce placeholder generated by map loader when actual text resource not found!",SpellLang::ENG,SpellTextRec::TextPanel::DEFAULT,NULL,true);
...
int SpellMap::Render(wxBitmap &bmp, TScroll* scroll, SpellTool *tool,std::function<void(void)> hud_buttons_cb)
{
	...
	for(i = 0; i < L5.size(); i++)
	{
		...
		int mxx = (pos.x - xs_ofs)*80 + ((((pos.y - ys_ofs*2) & 1) != 0) ? 0 : 40);
		int myy = (pos.y - ys_ofs * 2) * 24 - y_elev*18 + MSYOFS + 50;
		mark->Render(pic,pic_end,mxx,myy,pic_x_size,fil);
	}*/
	...
	while(open_set.size())
	{
		...
		for(int nid = 0; nid < 8; nid++)
		{
			...
			else if(!unit->unit->isAir() && !unit->unit->usingTeleportMove())
			{
				...
				double cost = 1.0;
				if(class_flags & Sprite::LandFlags::IS_DIRT_ROAD)
					cost = 0.9;
				else if(class_flags & Sprite::LandFlags::IS_ROAD)
					cost = 0.75;
				ap_step *= cost;
			}
						...
						if(phase == 0)
						{
							...
							{							
								...
								for(int k = 0; k < neig->GetContextCount(eid180,ref_slope); k++)
								{
									...
									int is_object = cspr->GetFlags() & Sprite::LandFlags::IS_OBJECT;
									//int is_object = true;
...
int SpellMap::EditClassTile(MapXY pos,std::vector<int> &modz,uint32_t target_flags,uint32_t blend_flags,uint32_t land_type_mask,int mod_level)
{
	...
	int mod_state = 0;
	if(mod_level > 1 && flags & target_flags)
	{
		// tile to full target class
		flags |= target_flags;
		flags &= ~land_type_mask;
		mod_state = 2;
	}
	else if(mod_level)
	{
...
int SpellMap::EditClass(vector<MapXY>& selection, SpellTool *tool, std::function<void(std::string)> status_cb)
{
	...
	uint32_t land_type_mask = Sprite::LandFlags::IS_GRASS | Sprite::LandFlags::IS_DGRASS | Sprite::LandFlags::IS_BLOOD | Sprite::LandFlags::IS_MUD | Sprite::LandFlags::IS_SWAMP | Sprite::LandFlags::IS_ASH | Sprite::LandFlags::IS_SAND | Sprite::LandFlags::IS_HIGHLAND | Sprite::LandFlags::IS_WATER;
	std::vector<uint32_t> land_classes = {Sprite::LandFlags::IS_GRASS,Sprite::LandFlags::IS_DGRASS,Sprite::LandFlags::IS_BLOOD,Sprite::LandFlags::IS_MUD,Sprite::LandFlags::IS_SWAMP,Sprite::LandFlags::IS_ASH,Sprite::LandFlags::IS_SAND,Sprite::LandFlags::IS_HIGHLAND,Sprite::LandFlags::IS_WATER};
